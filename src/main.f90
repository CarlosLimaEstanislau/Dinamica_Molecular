program simulation
    ...
    if (len_trim(params%outdir) == 0) then
        error stop "Error: Diretorio de saída não encontrado!"
    end if
    ...
    do step = 1, params%max_steps
        ...
        if (mod(step, 100) == 0) then
            ...
            call write_pdb(sys, part, params%sim_filename, params%outdir)
            call write_log(sys, part, params%sim_filename, params%outdir, step, temp, params%is_first)
        ...
        if (params%interrupt .or. step == params%max_steps) then
            call write_final_config(sys, part, params%sim_filename, params%outdir)
            exit
        end if
